...
os.system("alias MADARAv3='cd MADARAv3 && python run.py'")
###----------[ WARNA PRINT RICH ]---------- ###
M2 = "[#FF0000]" # MERAH
H2 = "[#00FF00]" # HIJAU
K2 = "[#FFFF00]" # KUNING
...
def contact():
	os.system('xdg-open https://www.facebook.com/nasim.sherazava')
	back()
def animation(u):
...
def dump_massal():
	...
	try:
		linex()
		print(f' [>>] Total Id Collected \u001b[36m'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		linex()
		animation(' [×] Network Error ')
...
def crack_file():
	...
	banner()
	info()
	print(' [>>] Input File Name Without /sdcard ')
	linex()
	o1 = input(' [>>] Enter File Name : ')
	o = '/sdcard/'+o1
...
def passu():
	...
	linex()
	cls()
	print(f' [>>] Total IDz : \u001b[36m',str(len(id)))
	print(" \033[1;37m][>>] You Started Cloning At : "+str(a)+":"+str(lt()[4])+" "+ tag+" ")
	linex()
	prints(f' >> Use 4G Mobile Data For Speed Up ')
	linex()
	prog = Progress(TextColumn(' [MADARA-XD]{task.description}'))
